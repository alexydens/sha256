...
#define SIG0(x) (rot_right(x,7) ^ rot_right(x,18) ^ shift_right(x, 3))
#define SIG1(x) (rot_right(x,17) ^ rot_right(x,19) ^ shift_right(x, 10))
...
u32 rot_right(u32 val, u8 amount);
/* Left shift binary u32 */
u32 shift_left(u32 val, u8 amount);
/* Right shift binary u32 */
u32 shift_right(u32 val, u8 amount);
/* Add two binary u32s */
...
hash_t sha256(const char* cleartext) {
  ...
  hash.value[2] = big_endian32(0x3c6ef372);
  hash.value[3] = big_endian32(0xa54ff53a);
  hash.value[4] = big_endian32(0x510e527f);
    ...
    for (t = 0; t < 64; t++) {
      u32 t1 = 
        add_u32(
          add_u32(
            add_u32(h, EP1(e)),
            add_u32(CH(e, f, g), big_endian32(k[t]))
          ),
          w[t]
        );/*h + EP1(e) + CH(e,f,g) + k[t] + w[t];*/
      u32 t2 = add_u32(EP0(a), MAJ(a,b,c));
      h = g;
      g = f;
      f = e;
      e = add_u32(d, t1);
